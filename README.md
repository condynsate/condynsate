...
Documentation, tutorials, and examples were generated with the intent of educating instructors on the use of condynsate to develop teaching demonstrations. The five included tutorials walk through the development of condynsate-based projects, including the simulation environment, design and implementation of mechanical systems, application of internal and external forces and torques to the mechanical systems, real-time visualization and animation, keyboard interactivity, and methods of data collection. In addition, at least one file giving an example of usage is included for each of the major modules including the Keyboard module, which provides keyboard interactivity, the Visualizer module, which provides 3D visualization, the Animator module, which provides GUI-based plotting, the Simulator module, which provides the physics simulation environment, and the Project module, which provides automatic interfacing with all other modules for easy project development. The Project examples are an inverted pendulum autonomously balanced on a 4-wheeled cart, a three-axis gyroscope with keyboard interactivity, and a double pendulum with real-time phase space plotting. These specific examples were chosen because they are common mechanical systems used in dynamics and control education. 
