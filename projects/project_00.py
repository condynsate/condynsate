# -*- coding: utf-8 -*-
"""
This module implements the backend for the wheel on axel project. In this
project, we apply torques to the to a wheel on an freely rotating axle to
enforce a target rotational angle.
"""
"""
...
if __name__ == "__main__":
    ...
    proj.visualizer.set_ptlight_1(on=True,
                                  position=(-4, -4, 4),
                                  intensity=0.3,
                                  shadow=True)
    proj.visualizer.set_ptlight_2(on=True,
                                  position=(-1, -1, 3),
                                  intensity=0.3,
                                  shadow=True)
    proj.visualizer.set_amblight(on=True,
                                 intensity=0.7,
                                 shadow=False)
    ...
    while proj.simtime <= 2.0:
        # apply_torques(proj, gyro)
        # set_omega(proj, gyro)
        # set_color(gyro)

        wheel.joints['axle_to_wheel'].apply_torque(0.5)
